...
import com.exchange.mod.core.DeliveryContract;
...
import com.exchange.mod.datapack.ContractTemplate;
...
public class DatapackRegistryTest {
    ...
    public void testBuiltinCommodityFiles() {
        ...
        for (String c : commodities) {
            String path = "/data/exchange/commodities/" + c + ".json";
    ...
    public void testBuiltinMarketEventFiles() {
        ...
        for (String ev : events) {
            String path = "/data/exchange/market_events/" + ev + ".json";
            InputStream is = getClass().getResourceAsStream(path);
    ...
    public void testBuiltinDeliveryContractFiles() {
        ...
        for (String ctr : contracts) {
            String path = "/data/exchange/delivery_contracts/" + ctr + ".json";
            InputStream is = getClass().getResourceAsStream(path);
    ...
    public void testDynamicContractGeneration() throws SQLException {
        ContractTemplate copperTemplate = new ContractTemplate(
                "copper_mega", "Mega Copper Delivery", "contract.exchange.copper_industrial.title",
    ...
    @Test
    @DisplayName("ExchangeConfig de-hardcoded helper methods provide safe fallbacks")
    public void testConfigFallbacks() {
        assertEquals(0.08, ExchangeConfig.getSurplusBurnRate(), 0.001);
        assertEquals(0.0005, ExchangeConfig.getDailyCarryFeeRate(), 0.00001);
        assertEquals(500, ExchangeConfig.getRankThreshold(2));
        assertEquals(2500, ExchangeConfig.getRankThreshold(3));
        assertEquals(10000, ExchangeConfig.getRankThreshold(4));
        assertEquals(50000, ExchangeConfig.getRankThreshold(5));

        // Vending capacity upgrades
        assertEquals(250.0, ExchangeConfig.getCapacityUpgradeCost(64), 0.01);
        assertEquals(500.0, ExchangeConfig.getCapacityUpgradeCost(128), 0.01);
        assertEquals(1000.0, ExchangeConfig.getCapacityUpgradeCost(256), 0.01);
        assertEquals(2500.0, ExchangeConfig.getCapacityUpgradeCost(512), 0.01);

        // Vending slot unlocks
        assertEquals(100.0, ExchangeConfig.getSlotUnlockCost(6), 0.01);
        assertEquals(200.0, ExchangeConfig.getSlotUnlockCost(7), 0.01);
        assertEquals(300.0, ExchangeConfig.getSlotUnlockCost(8), 0.01);
        assertEquals(400.0, ExchangeConfig.getSlotUnlockCost(9), 0.01);
        assertEquals(500.0, ExchangeConfig.getSlotUnlockCost(10), 0.01);

        // Satellite module
        assertEquals(500.0, ExchangeConfig.getSatelliteModuleCost(), 0.01);
    }
}
