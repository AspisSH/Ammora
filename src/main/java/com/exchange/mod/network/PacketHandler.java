                ...
                (payload, context) -> context.enqueueWork(() -> {
                    if (context.player() instanceof ServerPlayer serverPlayer) {
                        if (payload.pinForRedstone()) {
                            ...
                            var mr = ExchangeMod.getMarketManager() != null ? ExchangeMod.getMarketManager().getResource(payload.resourceId()) : null;
                            String name = mr != null ? mr.getDisplayName() : payload.resourceId();
                            OrderPacketHandler.sendMarketDataToClient(serverPlayer, payload.resourceId(), ExchangeLang.notify("terminal.redstone_pinned", name, 15), false);
                        } else {
