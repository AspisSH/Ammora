 ...
 */
public class AmmoraPeripheral implements IPeripheral {

    private final Object targetEntity;
    private final String type;

    public AmmoraPeripheral(Object entity) {
    ...
    public final Map<String, Object> getPrice(String itemId) {
        ...
        MarketResource res = AmmoraMod.getMarketManager().getResource(itemId);
        if (res == null) {
    ...
    public final Map<String, Object> getAccount(String playerUuidStr) {
        ...
        try {
            ...
            PlayerAccount acc = AmmoraMod.getMarketDAO().getAccount(uuid, "Trader");
    ...
    public final Map<String, Object> buy(String playerUuidStr, String itemId, int count) {
        ...
        try {
            UUID uuid = UUID.fromString(playerUuidStr);
            var result = AmmoraMod.getMarketManager().executeBuy(uuid, "Trader", itemId, count);
    ...
    public final Map<String, Object> sell(String playerUuidStr, String itemId, int count) {
        ...
        try {
            UUID uuid = UUID.fromString(playerUuidStr);
            var result = AmmoraMod.getMarketManager().executeSell(uuid, "Trader", itemId, count);
            map.put("success", result.success());
            map.put("message", result.message());
    ...
    public final List<Map<String, Object>> getCandles(String itemId, int limit) {
        ...
        if (AmmoraMod.getMarketDAO() == null) return list;

        try {
            List<Candle> candles = AmmoraMod.getMarketDAO().getRecentCandles(itemId, "1d", Math.min(50, limit));
    ...
    public boolean equals(@Nullable IPeripheral other) {
        return other instanceof AmmoraPeripheral p && p.targetEntity == this.targetEntity;
    }
}
